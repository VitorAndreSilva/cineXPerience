...
<template>
    <div class="flex justify-center">
        <input type="text" v-model="search" @input="filterBySearch" placeholder="Buscar título..." class="border bg-[#94A3B8]-300 rounded-sm w-122 h-9 mx-2 mt-7 mb-7 p-4">
        <select v-model="filterYear" @change="filterByYear" class="border bg-[#94A3B8]-300 rounded-sm w-122 h-9 mx-2 mt-7 mb-7">
            <option value="">Todos</option>
            <option v-for="year in years" :key="year.id" :value="year">{{ year }}</option>
        </select>
        <select v-model="filterGenre" @change="filterByGenre" class="border bg-[#94A3B8]-300 rounded-sm w-122 h-9 mx-2 mt-7 mb-7">
            <option value="">Todos</option>
